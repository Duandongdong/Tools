...
package com.baigu.ui.widget.filterlistview;
...
public class IndexBarView extends View {
	...
	public void setData(PinnedHeaderListView listView,
			ArrayList<String> listItems, ArrayList<Integer> listSections) {
		...
		mIndexbarMargin = mContext.getResources().getDimension(
		...
		mIndexPaint.setTextSize(mContext.getResources().getDimension(
	...
	protected void onDraw(Canvas canvas) {
		if (mListSections != null && mListSections.size() > 1) {
			float sectionHeight = (getMeasuredHeight() - 2 * mIndexbarMargin)
					/ mListSections.size();
			float paddingTop = (sectionHeight - (mIndexPaint.descent() - mIndexPaint
					.ascent())) / 2;
			...
			for (int i = 0; i < mListSections.size(); i++) {
				...
				canvas.drawText(getSectionText(mListSections.get(i)),
						paddingLeft, mIndexbarMargin + (sectionHeight * i)
	...
	void filterListItem(float sideIndexY) {
		...
		mCurrentSectionPosition = (int) (((mSideIndexY) - getTop() - mIndexbarMargin) / ((getMeasuredHeight() - (2 * mIndexbarMargin)) / mListSections
				.size()));
